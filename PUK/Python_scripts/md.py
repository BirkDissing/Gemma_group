...
dyn = NVTBerendsen(
    mol, timestep=2.0 * units.fs, temperature=T * units.kB, taut=0.5 * 1000 * units.fs, logfile="EtOH_moldyn_NVT.log"
)
...
force_array = np.zeros([len(traj), len(traj[0]), 3])
for atoms, i in enumerate(traj):
